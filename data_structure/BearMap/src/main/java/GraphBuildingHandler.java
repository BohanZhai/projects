...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    private Long from;
    private LinkedList<Long> adjrecord;
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            GraphDB.Vertice v = new GraphDB.Vertice(attributes.getValue("id"),
            ...
            activeState = "way";
            GraphDB.Edge e = new GraphDB.Edge(attributes.getValue("id"));
            from = e.id;
            //g.addEdge(e.id, e);
            adjrecord = new LinkedList<>();

        } else if (activeState.equals("way") && qName.equals("nd")) {
            Long nd = Long.valueOf(attributes.getValue("ref"));
            GraphDB.Vertice v = g.getVertice(nd);
            //v.way.add(e.id);
            //e.waynodes.add(nd);
            adjrecord.add(nd);
            ...
            /*if (k.equals("maxspeed")) {
                ...
                //GraphDB.Edge e = g.getEdge(from);
                //e.tag.put("maxspeed", v);
                //e.maxspeed = v;
            //}
            if (k.equals("highway")) {
                ...
                if (ALLOWED_HIGHWAY_TYPES.contains(v)) {
                    //GraphDB.Edge e = g.getEdge(from);
                    Long past = null;
                    for (Long i : adjrecord) {
                        if (past == null) {
                            past = i;
                            continue;
                        }
                        GraphDB.Vertice v1 = g.getVertice(past);
                        GraphDB.Vertice v2 = g.getVertice(i);
                        v1.adjacentid.add(i);
                        v2.adjacentid.add(past);
                        past = i;
                    }
                    adjrecord = new LinkedList<>();
                } else {
            ...
            GraphDB.Vertice v = g.getVertice(from);
            //v.tag.put("name", attributes.getValue("v"));
            v.name = attributes.getValue("v");
            if (v.name.contains("piz")) {
