...
def eval_all(expressions, env):
    ...
    while expressions != nil: #eval all the expressions
...
def do_and_form(expressions, env):
    ...
    if len(expressions)==0:
        ...
        temp=scheme_eval(expressions.first, env,True)
        if not scheme_truep(temp):
            return False
        elif len(expressions)==1 and scheme_truep(temp):
            return temp
        else:
