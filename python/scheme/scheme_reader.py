...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            src.remove_front() # remove each val after we read it.
            return nil
            # END PROBLEM 1
        elif src.current() == '.':
            # BEGIN PROBLEM 2
            src.remove_front()
            temp_read_result=scheme_read(src) #we need to check if there is ')' following
            if src.remove_front() == ')':  
            ...
            # BEGIN PROBLEM 1
            return Pair(scheme_read(src), read_tail(src)) # use scheme_read to deal with ( and values and read_tail to creat Pair
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
