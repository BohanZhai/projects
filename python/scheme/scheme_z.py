...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    if expressions is nil: #return None when there is no elements
        return None
    elif expressions.second is nil: # return the last evaluation 
        return scheme_eval(expressions.first, env)
    else:
        scheme_eval(expressions.first ,env) # evlaueate each elements
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals)!=len(vals):
            raise SchemeError
        while formals != nil:
            child.define(formal.first, vals.first)
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        #args are arguments and scheme_eval can only evalue one element at a time
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            return do_cond_form(expressions.second, env)
            # END PROBLEM 14
