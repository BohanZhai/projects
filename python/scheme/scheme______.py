...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        return PrimitiveProcedure.apply(self, args, env)
        # END PROBLEM 5
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        formals = expressions.first
        check_formals(formals)
        env.define(target.first, MacroProcedure(formals, expressions.second, env)) #expand the bindings of the env by calling define method
        return target.first # return the name of the procedure
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            formals=env.bindings[first].formals
            env_p=env.bindings[first].env.
            procedure = scheme_eval(first, env)
            operands = rest.map(lambda element: scheme_eval(element, env))
            result = scheme_apply(procedure, operands, env)
            # END PROBLEM 20
    return result
