...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
            elif self.ant.container==True and self.ant.ant is None and insect.container != True:
                self.ant.ant=insect
            elif self.ant.container!=True and insect.can_contain and insect.ant==None:
                insect.ant=self.ant
                self.ant=insect
            else:           
    ...
    def remove_insect(self, insect):
        ...
        """
        is_queen = isinstance(insect,QueenAnt) and insect.Queen
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                if not (isinstance(self.ant.ant, QueenAnt) and self.ant.ant.Queen == True): 
                    self.ant.ant = None
            else:
                assert False, '{0} is not in {1}'.format(insect, self)
        else:
            self.bees.remove(insect)
        if not isinstance(insect, QueenAnt) or insect.Queen != True:
            insect.place = None
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is not None:
            if not self.place.ant.blocks_pass:
                return False
    ...
    def nearest_bee(self,hive):
        ...
        while p.entrance != hive:
            if p.bees==[]:
                p=p.entrance
                index += 1
            elif index <= self.max_range  and index >= self.min_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting!=0:
            ...
            self.eat_bee(self.random_or_none())
    def random_or_none(self):
        if self.place.bees:
            return random.choice(self.place.bees)
        # END Problem 10
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if self.Queen:
            super().__init__()
            QueenAnt.Queen=False
            self.Queen=True
            #print("queen is " + str(self.Queen)+" fake one is "+str(QueenAnt.Queen))
        else:
            super().__init__()       
        # END Problem 13
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        antplace=self.place
        ant_next_place=self.place.exit
        if self.Queen == False:
            self.reduce_armor(self.armor)
        else:
            while antplace!=None:
                if antplace.ant !=None :
                    if antplace.ant.container==False and not isinstance(antplace.ant,QueenAnt) and antplace.ant not in self.double_ants:
                        antplace.ant.damage=antplace.ant.damage*2
                        self.double_ants.append(antplace.ant)
                        
                    elif antplace.ant.container==True and antplace.ant.ant!=None: 
                        if antplace.ant.ant not in self.double_ants:
                            antplace.ant.ant.damage=antplace.ant.ant.damage*2
                            self.double_ants.append(antplace.ant.ant)
                        if antplace.ant not in self.double_ants:
                            antplace.ant.damage=antplace.ant.damage*2
                            self.double_ants.append(antplace.ant)
                    elif antplace.ant.container==True and antplace.ant.ant==None and antplace.ant not in self.double_ants:
                        antplace.ant.damage=antplace.ant.damage*2
                        self.double_ants.append(antplace.ant)
                antplace=antplace.exit
