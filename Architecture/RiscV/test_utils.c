...
//own test
void test_parse_instruction_slli();
void test_parse_instruction_jalr();
...
int main(int arc, char **argv) {
    ...
    if (!CU_add_test(pSuite1, "test_parse_instruction_slli", test_parse_instruction_slli)) {
        goto exit;
    }

    if (!CU_add_test(pSuite1, "test_parse_instruction_jalr", test_parse_instruction_jalr)) {
        goto exit;
    }
...
void test_parse_instruction_slli() {
    Instruction inst;
    inst = parse_instruction(0x00241413);
    CU_ASSERT_EQUAL(inst.itype.opcode, 0x13);
    CU_ASSERT_EQUAL(inst.itype.funct3, 0x01);
    CU_ASSERT_EQUAL(inst.itype.rd, 0x8);
    CU_ASSERT_EQUAL(inst.itype.rs1, 8);
    CU_ASSERT_EQUAL(inst.itype.imm, 2);
}

void test_parse_instruction_jalr() {
    Instruction inst;
    inst = parse_instruction(0x12308067);
    CU_ASSERT_EQUAL(inst.itype.opcode, 0x67);
    CU_ASSERT_EQUAL(inst.itype.funct3, 0x0);
    CU_ASSERT_EQUAL(inst.itype.rd, 0x0);
    CU_ASSERT_EQUAL(inst.itype.rs1, 1);
    CU_ASSERT_EQUAL(inst.itype.imm, 0x123);
}
