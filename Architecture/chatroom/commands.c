...
void handle_set_nickname (char **args, unsigned count, unsigned n) {
	...
	if (!((isword (args[0]) && strlen (args[0]) <= MAX_NAME_LENGTH) && isvalidname(args[1]))) {
		handle_invalid_arguments("set_nickname", n);
		return;
	}
	if (!user) {
	...
	char *temp = (char *)malloc(strlen(args[1]) + 1);
	strcpy(temp, args[1]);
	temp[strlen(args[1])] = '\0';
	*t = temp;
	(user -> nickname) = t;
...
void handle_mute (char **args, unsigned count, unsigned n) {
	...
	char *muted_name = args[0];
	struct user_info *muted_user = find_user(muted_name);
	struct user_info *user = users[n];
	if (!muted_user){
		handle_invalid_arguments ("mute", n);
		return;
	}
	if (find_user(args[0]) == users[n]) {
		handle_invalid_arguments ("mute", n);
		return;
	}
	...
	if (!ismuted(user, muted_user)) {
		int total = user -> muted_capacity;
		int i = 0;
		while (i < total) {
			if ((user -> muted)[i] == NULL) {
				(user -> muted)[i] = muted_user -> name_info;
				break;
			}
			i++;
		}
...
void handle_unmute (char **args, unsigned count, unsigned n) {
	...
	char * unmuted_name = args[0];
	struct user_info *muted_user = find_user(unmuted_name);
	if (!muted_user) {
		handle_invalid_arguments ("unmute", n);
		return;
	}
	if (muted_user == users[n]) {
		handle_invalid_arguments ("unmute", n);
		return;
	}
	...
	int i = 0;
	int crt = 0;
	int total = user -> muted_total;
	while (i < total) {
		if ((user -> muted)[crt] == muted_user -> name_info) {
			(user -> muted)[crt] = NULL;
			break;
		}
		if ((user -> muted)[crt] != NULL) i++;
		crt ++;
...
void handle_show_status (char **args, unsigned count, unsigned n) {
	...
	char * showed_name = args[0];
	struct user_info *showed_user = find_user(showed_name);
	if (showed_user) {	
		//struct user_info *user = users[n];
		output_user_status (showed_user, n);
	}else {
		handle_invalid_arguments("show_status", n);
	}
}
