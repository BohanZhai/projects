...
char *generate_message (char *messages, int end) {
	/* YOUR CODE HERE. */
	if (*messages) {
		char *fstmessage = (char*) malloc(end + 1);
		if (!fstmessage) {
			allocation_failed();
		}
		int mlen = strlen(messages); // Get the length of messages
		char tempstring[mlen - end + 1]; // using for update the remaining messages
		strncpy(fstmessage, messages, end);
		*(fstmessage + end) = '\0';
		char *remain = messages + end;
		strcpy (tempstring, remain);
		strcpy (messages, tempstring);
		return fstmessage;
	}
        return NULL;
}
